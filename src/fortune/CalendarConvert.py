...
def Gregorian2Lunar(wCurYear,wCurMonth,wCurDay):
    #公历每月前面的天数 days to date before starting month
    ...
    # how did the math change between Py3 and Py2?
    nTheDate = (wCurYear - 1901) * 365 + int((wCurYear - 1901) / 4) + wCurDay + wMonthAdd[wCurMonth - 1] - 38-11
    if nTheDate<=0:
    ...
    if k==12:
        if wCurMonth==int(wNongliData[m]/65536)+1:
            wCurMonth=1-wCurMonth
        elif wCurMonth>int(wNongliData[m]/65536)+1:
...
def Get8Zi(nGreYear,nGreMonth,nGreDay,nHour,nMinute):
    wLunarYear, wLunarMonth, wLunarDay, Embolism = Gregorian2Lunar(nGreYear, nGreMonth, nGreDay)
    sShuxiang, sTianGan, sDiZhi, sNongliMonth, sNongliDay = GetLunarString(wLunarYear, wLunarMonth, wLunarDay, Embolism)
    ...
    nA=(nGongYuanYear%12)*5  # todo: indent?
    nB=int(nGongYuanYear/4)  # todo: indent?
    ...
    str_Time=cListTime[int(nShiCheng)][int(nTodayGan%5)]
    #考刻分 时上起刻
    nTimeGan=0
    for j in range(0,10):
        if str_Time[:1]==cTianGan[j]:
            nTimeGan=j
            break
    str_Minute=cListTime[int(nFeng)][int(nTimeGan%5)]
...
#         wLunarYear, wLunarMonth, wLunarDay, Embolism = Gregorian2Lunar(2012,6,15)
#         for a in GetLunarString(wLunarYear, wLunarMonth, wLunarDay, Embolism):
#             print(a)
